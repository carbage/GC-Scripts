...
public class GcPowerMiner extends ActiveScript implements MessageListener {
	...
	private final List<Node> fridge = Collections.synchronizedList(new ArrayList<Node>());
	...
	public void onStart() {
		...
		miningExp = Skills.getExperience(Skills.MINING);
	...
	public int loop() {
		...
		new Runnable() {
			public void run() {
				if (getData() != null && gui != null) gui.updateRows(getData());
			}
		};
	...
	private Object[][] getData() {
		if (gui != null) { // Checks if GUI has been initialised
			return new Object[][] { { "Ores mined:", getOresMined() },
					...
					{ "Ores per hour:", perHour(getOresMined()) },
					{ "Exp per hour:", perHour(getExpGained()) } };
		}
	...
	private int getExpGained() {
		return Skills.getExperience(Skills.MINING) - miningExp;
	}
